...
class Employee(models.Model):
    ...
    photo=models.ImageField(default='contact_photos/default_photo.png',upload_to='contact_photos/',blank=True)
    job_description = models.TextField(default='staff')
    def __str__(self):
        return self.user.username
...
class SliderSettings(models.Model):
    ...
    def __str__(self):
        return f"Настройки слайдера (delay: {self.delay}s, auto: {self.auto}, loop: {self.loop}, navs: {self.navs}, pags: {self.pags}, stopMouseHover: {self.stopMouseHover})"