...
from ..forms import  AdminSignUpForm,MedicationForm,PharmacyDepartmentForm,SupplierForm,MedicationSearchForm,SliderSettingsForm
from ..models import  User,Medication,PharmacyDepartment,Supplier,Sale,Employee,Customer,SliderSettings
...
def view_medications(request):
    ...
    #         "photo": medication.photo.url if medication.photo else ''
    #     }
...
def slider_settings(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('set_banner_settings')
    else:
        form = SliderSettingsForm(instance=settings)

    return render(request, 'set_banner_settings.html', {'form': form})
